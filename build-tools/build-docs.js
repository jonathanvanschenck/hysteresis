...
glob.sync(path.resolve("*.js")).forEach((fp)=> {
...
data = jsdoc2md.getTemplateDataSync({files: files, configure: path.resolve("build-tools/jsdoc-config.json") })
set_empty_string(data)
output = jsdoc2md.renderSync({ data: data, template: fs.readFileSync(path.resolve("build-tools/api-template.hbs"),'utf-8') })
